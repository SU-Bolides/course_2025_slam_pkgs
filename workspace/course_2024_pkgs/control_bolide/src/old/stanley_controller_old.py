...
class StanleyController:
    ...
    def __init__(self):
        ...
        self.STEERING_LIMIT_DEG = float(rospy.get_param("~steering_limit_deg", "13.5"))
    ...
    def carstateCB(self, data):
        if not rospy.is_shutdown():
            self.measured_dir_deg = data.direction
            self.measured_velocity = data.speed


    def starting_check_timer(self, evt):
        #Check to start:
        try:
    ...
    def drive_to_target_stanley(self):
        ...
        angle = np.clip(angle, -np.radians(self.STEERING_LIMIT_DEG), np.radians(self.STEERING_LIMIT_DEG))
    ...
    def odomCB(self, pose_msg: Odometry):
        ...
        # self.old_target_heading = self.target_heading
