...
def detect_clusters(occupancy_grid, eps=3, min_samples=10):
    ...
    """
    shape = np.shape(occupancy_grid)
    # Réduction du bruit avec un filtre médian
    structure = generate_binary_structure(2, 2)  # 2D connectivity
    occupancy_grid_filtered = binary_closing(binary_dilation(occupancy_grid, structure=structure), structure=structure)
    # Trouve les points occupés
    points = np.argwhere(occupancy_grid_filtered == True)
    ...
    for label in set(labels):
        if label != -1:  # Ignore les points considérés comme du bruit
            ...
            print(len(xy))
            if len(xy) > 50 and len(xy) < 60:  # Filtre les clusters trop petits ou trop grands
                x = round(np.mean(xy[:, 0]))
                y = round(np.mean(xy[:, 1]))
                clusters.append(grid_to_rel((x,y), shape))
    return clusters


def grid_to_rel(grid_point, shape):
    print(grid_point)
    carpos = (shape[0]//2, 0)
    x = grid_point[0] - carpos[0]
    y = grid_point[1] - carpos[1]
