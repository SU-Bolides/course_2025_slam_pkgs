...
class OdomForkImu(Node):
    def __init__(self):
        ...
        self.sub_vel = self.create_subscription(SpeedDirection, '/cmd_vel', get_speedsign)
        self.sub_raw_imu_data = self.create_subscription(Imu, '/raw_imu_data', get_dir)
        self.sub_gps_odom = self.create_subscription(Odometry, '/gps/odom', get_gps)
