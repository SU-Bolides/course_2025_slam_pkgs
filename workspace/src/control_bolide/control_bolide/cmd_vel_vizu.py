...
import cv2
import rclpy
from control_bolide_msgs.msg import SpeedDirection
from rclpy.node import Node

class CmdVelVisualisation(Node) :
    def __init__(self, mode = 2) :
        super().__init__('cmd_vel_vizu')
        self.mode = mode
...
def main(args=None):
    ...
    cmd_vel_vizu = CmdVelVisualisation()
    rclpy.spin(cmd_vel_vizu)
    cmd_vel_vizu.destroy_node()
    rclpy.shutdown()
