...
class LidarVizu(Node):
    def __init__(self, rmax:int=7):
        ...
        self.sub_raw_lidar_data = self.create_subscription(LaserScan, "/raw_lidar_data", self.callback_raw_data, 10)
        self.sub_lidar_data = self.create_subscription(LaserScan, "/lidar_data", self.callback_processed_data, 10)
        self.sub_param_change_alert = self.create_subscription(Bool, "/param_change_alert", self.get_rmax, 10)
