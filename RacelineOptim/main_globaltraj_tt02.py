...
by the stanley controller on bolide1. 
"""
...
parser = argparse.ArgumentParser(description='Generate optimal trajectory for TT02 racing.')
parser.add_argument('--map_name', type=str, default='esclangon_loop_2', help='Name of the map (default: esclangon_loop_2)')
...
# file_paths = {"veh_params_file": "tt02.ini"}
file_paths = {"veh_params_file": "tt02.ini"}
...
lap_time_mat_opts = {"use_lap_time_mat": False,             # calculate a lap time matrix (diff. top speeds and scales)
                     ...
                     "top_speed_range": [5.0, 10.0],        # range of top speeds to be simulated [in km/h]
...
file_paths["traj_ltpl_export"] = os.path.join(file_paths["module"], f"outputs/{MAP_NAME}", f"traj_race_cl_ltpl-{TIME}.csv")
